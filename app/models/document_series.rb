class DocumentSeries < ActiveRecord::Base
  ...
  def latest_editions
    @latest_editions ||= documents.map(&:latest_edition).compact.sort {|edition| edition.publication_date.to_i }
  end
  ...
  def published_editions
    editions_through_new_scope = editions.published.order('publication_date DESC')
